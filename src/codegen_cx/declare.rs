...
use rustc_middle::{
    ...
    mir::mono::{Linkage, MonoItem, Visibility},
    ty::{
        self,
        layout::{FnAbiOf, LayoutOf},
        Instance, ParamEnv, TypeVisitableExt,
    },
};
...
use crate::{
    abi::ConvSpirvType,
    assert_ty_eq,
    attr::AggregatedSpirvAttributes,
    builder_spirv::{SpirvConst, SpirvValue, SpirvValueExt},
    custom_decorations::{CustomDecoration, SrcLocDecoration},
...
impl<'tcx> CodegenCx<'tcx> {
    ...
    pub fn get_static(&self, def_id: DefId) -> SpirvValue {
        ...
        assert!(
            ...
            "get_static() should always hit the cache for statics defined in the same CGU, but \
             did not for `{def_id:?}`"
        );
...
impl<'tcx> PreDefineMethods<'tcx> for CodegenCx<'tcx> {
    fn predefine_static(
        ...
        let linkage = match linkage {
            ...
            other => {
                self.tcx.sess.psess.dcx.err(format!(
                    "TODO: Linkage type {other:?} not supported yet for static var symbol \
                     {symbol_name}"
                ));
                None
            }
    ...
    fn predefine_fn(
        ...
        let linkage2 = match linkage {
            ...
            other => {
                self.tcx.sess.psess.dcx.err(format!(
                    "TODO: Linkage type {other:?} not supported yet for function symbol \
                     {symbol_name}"
                ));
                None
            }
