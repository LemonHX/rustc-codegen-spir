...
use rustc_codegen_ssa::{
    mir::debuginfo::{FunctionDebugContext, VariableKind},
    traits::{AsmMethods, BackendTypes, DebugInfoMethods, GlobalAsmOperandRef, MiscMethods},
};
...
use rustc_middle::{
    ...
    ty::{
        ...
        Instance, ParamEnv, PolyExistentialTraitRef, Ty, TyCtxt,
    },
};
use rustc_session::Session;
use rustc_span::{symbol::Symbol, SourceFile, Span, DUMMY_SP};
use rustc_target::{
    abi::{
        call::{FnAbi, PassMode},
        AddressSpace, HasDataLayout, TargetDataLayout,
    },
    spec::{HasTargetSpec, Target},
};

use crate::{
    builder::{ExtInst, InstructionTable},
    builder_spirv::{BuilderCursor, BuilderSpirv, SpirvConst, SpirvValue, SpirvValueKind},
    custom_decorations::{CustomDecoration, SrcLocDecoration, ZombieDecoration},
    spirv_type::{SpirvType, SpirvTypePrinter, TypeCache},
    symbols::Symbols,
    target::SpirvTarget,
};
...
impl CodegenArgs {
    ...
    pub fn parse(args: &[String]) -> Result<Self, rustc_session::getopts::Fail> {
        ...
        opts.optflag(
            "",
            "relax-struct-store",
            "Allow store from one struct type to a different type with compatible layout and \
             members.",
        );
        opts.optflag(
            "",
            "relax-logical-pointer",
            "Allow allocating an object of a pointer type and returning a pointer value from a \
             function in logical addressing mode",
        );
        opts.optflag(
            "",
            "relax-block-layout",
            "Enable VK_KHR_relaxed_block_layout when checking standard uniform, storage buffer, \
             and push constant layouts. This is the default when targeting Vulkan 1.1 or later.",
        );
        opts.optflag(
            "",
        ...
        opts.optflag(
            "",
            "scalar-block-layout",
            "Enable VK_EXT_scalar_block_layout when checking standard uniform, storage buffer, \
             and push constant layouts. Scalar layout rules are more permissive than relaxed \
             block layout so in effect this will override the --relax-block-layout option.",
        );
        opts.optflag(
            "",
            "skip-block-layout",
