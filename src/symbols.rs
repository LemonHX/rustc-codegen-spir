...
use crate::{
    attr::{Entry, ExecutionModeExtra, IntrinsicType, SpecConstant, SpirvAttribute},
    builder::libm_intrinsics,
};
...
fn parse_local_size_attr(arg: &NestedMetaItem) -> Result<[u32; 3], ParseAttrError> {
    ...
    match arg.meta_item_list() {
        ...
        Some([]) => Err((
            arg.span,
            "#[spirv(compute(threads(x, y, z)))] must have the x dimension specified, trailing \
             ones may be elided"
                .to_string(),
        )),
        ...
        _ => Err((
            arg.span,
            "#[spirv(compute(threads(x, y, z)))] must have 1 to 3 parameters, trailing ones may \
             be elided"
                .to_string(),
        )),
    }
                ...
                return Err((
                    ...
                    String::from(
                        "The `threads` argument must be specified when using `#[spirv(compute)]`, \
                         `#[spirv(mesh_nv)]` or `#[spirv(task_nv)]`",
                    ),
                ));
            }
        }
