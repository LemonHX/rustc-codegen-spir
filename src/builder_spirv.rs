...
use rspirv::{
    ...
    dr::{Block, Builder, Module, Operand},
    spirv::{AddressingModel, Capability, MemoryModel, Op, SourceLanguage, StorageClass, Word},
};
...
use rustc_middle::{bug, mir::interpret::ConstAllocation, ty::TyCtxt};
use rustc_span::{
    source_map::SourceMap, symbol::Symbol, FileName, FileNameDisplayPreference, SourceFile, Span,
    DUMMY_SP,
};

use crate::{
    builder, codegen_cx::CodegenCx, spirv_type::SpirvType, symbols::Symbols, target::SpirvTarget,
    ...
    fn message(&self) -> &'static str {
        match *self {
            ...
            Self::UntypedConstDataFromAlloc => {
                "`const_data_from_alloc` result wasn't passed through `static_addr_of`, then \
                 `const_bitcast` (which would've given it a type)"
            }
        }
    }
}
