use rspirv::{
    dr,
    grammar::{reflect, LogicalOperand, OperandKind, OperandQuantifier},
    spirv::{
        FPFastMathMode, FragmentShadingRate, FunctionControl, ImageOperands, KernelProfilingInfo,
        LoopControl, MemoryAccess, MemorySemantics, Op, RayFlags, SelectionControl, StorageClass,
        Word,
    },
};
...
use rustc_codegen_ssa::{
    mir::place::PlaceRef,
    traits::{AsmBuilderMethods, InlineAsmOperandRef},
};
...
impl<'cx, 'tcx> Builder<'cx, 'tcx> {
        ...
        let new_result_id = match inst.class.opcode {
            ...
            Op::TypePointer => {
                ...
                if storage_class != StorageClass::Generic {
                    ...
                    err.note(format!("`{storage_class:?}` storage class was specified"));
                    err.help(format!(
            ...
            op => {
                ...
                if let Op::Return | Op::ReturnValue = op {
                    ...
                    err.note(
                        "resuming execution, without falling through the end of the `asm!` block, \
        ...
        'tcx: 'a,
    {
        ...
        let out_register = if match first_token {
            ...
            Token::Word(_) | Token::String(_) | Token::Typeof(..) => false,
        } {
                    ...
                    match place {
                        ...
                        None => {
                            self.tcx
                                ...
                                .psess
                                .dcx
                                .span_err(span, "missing place for register");
                            None
                        }
                    }
                }
        ...
        'tcx: 'a,
    {
        ...
        let word = match token {
            ...
            Token::String(_) | Token::Placeholder(..) | Token::Typeof(..) => None,
        };
        match (kind, word) {
