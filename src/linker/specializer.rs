...
use rspirv::{
    dr::{Builder, Function, Instruction, Module, Operand},
...
use crate::{
    linker::ipo::CallGraph,
    spirv_type_constraints::{self, InstSig, StorageClassPat, TyListPat, TyPat},
};
        ...
        for inst in &mut entry_points {
            ...
            for interface_operand in &mut inst.operands[3..] {
                ...
                match (instances.next(), instances.next()) {
                    (None, _) => unreachable!(
                        "entry-point %{} has overly-\"generic\" interface variable %{}, with no \
                         instances",
                        func_id, interface_id
                    ),
                    (Some(_), Some(_)) => unreachable!(
                        "entry-point %{} has overly-\"generic\" interface variable %{}, with too \
                         many instances: {:?}",
                        func_id,
                        interface_id,
                        FmtBy(|f| f
