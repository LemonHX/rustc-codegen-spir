...
use std::{cell::RefCell, collections::hash_map::Entry, fmt};
...
use rustc_middle::{
    ...
    ty::{
        ...
        Const, CoroutineArgs, FloatTy, GenericArgsRef, IntTy, ParamEnv, PolyFnSig, Ty, TyCtxt,
...
use rustc_target::{
    abi::{
        call::{ArgAbi, ArgAttributes, FnAbi, PassMode},
        Abi, Align, FieldsShape, LayoutS, Primitive, Scalar, Size, TagEncoding, VariantIdx,
        Variants,
    },
    spec::abi::Abi as SpecAbi,
};

use crate::{
    attr::{AggregatedSpirvAttributes, IntrinsicType},
            ...
            match pointee {
                Some(old_pointee) if old_pointee != new_pointee => {
                    cx.tcx.sess.psess.dcx.fatal(format!(
                        "dig_scalar_pointee: unsupported Pointer with different pointee types \
                         ({old_pointee:?} vs {new_pointee:?}) at offset {offset:?} in {layout:#?}"
                    ));
                }
        ...
        if let Variants::Single { index } = ty.variants {
            ...
            if i == 0 {
                ...
                cx.tcx
                    ...
                    .psess
                    .dcx
                    .fatal("Variants::Multiple has multiple fields")
            }
        };
    }
    SpirvType::Adt {
...
impl fmt::Display for TyLayoutNameKey<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let (TyKind::Coroutine(..), Some(index)) = (self.ty.kind(), self.variant) {
        ...
        IntrinsicType::Sampler => {
            ...
            if ty.size != Size::from_bytes(4) {
                return Err(cx
                    ...
                    .psess
                    .dcx
                    .err("#[spirv(sampler)] type must have size 4"));
            }
            ...
            match cx.lookup_type(elem_type) {
                ...
                ty => {
                    return Err(cx.tcx.sess.psess.dcx.span_err(
                        span,
                        format!(
                            "#[spirv(matrix)] type fields must all be vectors, found {}",
                            ty.debug(elem_type, cx)
                        ),
                    ));
                }
            }
