use std::{marker::PhantomData, mem, str};
...
use spirt::{
    ...
    Attr, AttrSet, AttrSetDef, Const, ConstCtor, Context, ControlNode, ControlNodeKind,
...
use crate::{
    custom_decorations::{CustomDecoration, SpanRegenerator, SrcLocDecoration, ZombieDecoration},
    custom_insts::{self, CustomInst, CustomOp},
};
        ...
        if extra > 0 {
            ...
            self.report_from_attrs(
                AttrSet::default().append_diag(
                    self.cx,
                    Diag::bug([format!(
                        "{extra} extraneous `use_stack` frame(s) found (missing \
                         `PopInlinedCallFrame`?)"
                    )
                    .into()]),
                ),
                                        ...
                                        Some(UseOrigin::IntraFunc { special_func, .. }) => {
                                            if let Some(SpecialFunc::Inlined { .. }) = special_func
                                            {
                                                ...
                                                self.report_from_attrs(
                                                    AttrSet::default().append_diag(
                                                        self.cx,
                                                        Diag::bug(["`PopInlinedCallFrame` \
                                                                    without an inlined call \
                                                                    frame in `use_stack`"
                                                            .into()]),
                                                    ),
                                                );
