...
use rustc_middle::{
    bug,
    mir::interpret::{alloc_range, ConstAllocation, GlobalAlloc, Scalar},
    ty::layout::LayoutOf,
};
...
use crate::{
    abi::ConvSpirvType,
    assert_ty_eq,
    builder_spirv::{SpirvConst, SpirvValue, SpirvValueExt, SpirvValueKind},
    ...
    fn read_from_const_alloc(
        ...
        *offset = offset.align_to(ty_concrete.alignof(self));
        ...
        match ty_concrete {
                ...
                if let Some(size) = size {
                    *offset += size;
                    ...
                    assert_eq!(
                        offset.bytes_usize(),
                        alloc.inner().len(),
                        "create_const_alloc must consume all bytes of an Allocation after an \
                         unsized struct"
                    );
                }
            ...
            SpirvType::Image { .. } => self
                .tcx
                .sess
                .psess
                .dcx
                .fatal("cannot create a constant image value"),
            ...
            SpirvType::RayQueryKhr => self
                .tcx
                .sess
                .psess
                .dcx
                .fatal("cannot create a constant ray query"),
        }
    }
}
