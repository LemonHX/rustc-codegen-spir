...
use rspirv::{
    ...
    spirv::{Capability, MemoryModel, MemorySemantics, Op, Scope, StorageClass, Word},
};
...
use rustc_codegen_ssa::{
    ...
    traits::{BackendTypes, BuilderMethods, ConstMethods, LayoutTypeMethods, OverflowOp},
    MemFlags,
};
...
use crate::{
    abi::ConvSpirvType,
    assert_ty_eq,
    builder_spirv::{BuilderCursor, SpirvConst, SpirvValue, SpirvValueExt, SpirvValueKind},
    custom_insts::{CustomInst, CustomOp},
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    fn ordering_to_semantics_def(&self, ordering: AtomicOrdering) -> SpirvValue {
        ...
        if invalid_seq_cst {
            self.zombie(
                semantics.def(self),
                "cannot use AtomicOrdering=SequentiallyConsistent on Vulkan memory model (check \
                 if AcquireRelease fits your needs)",
            );
        }
        semantics
    }
...
impl<'a, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'tcx> {
    ...
    simple_op! {unchecked_sadd, i_add}

    // already unchecked by default
    simple_op! {unchecked_uadd, i_add}

    // already unchecked by default
    simple_op! {unchecked_ssub, i_sub}

    // already unchecked by default
    simple_op! {unchecked_usub, i_sub}

    // already unchecked by default
    simple_op! {unchecked_smul, i_mul}

    // already unchecked by default
    simple_op! {unchecked_umul, i_mul}
            ...
            SpirvType::Integer(width, signed) => {
                let construct_case = match width {
                    ...
                    other => self.fatal(format!(
                        "switch selector cannot have width {other} (only 8, 16, 32, and 64 bits \
                         allowed)"
                    )),
                };
    ...
    fn not(&mut self, val: Self::Value) -> Self::Value {
        match self.lookup_type(val.ty) {
            SpirvType::Integer(..) => self.emit().not(val.ty, None, val.def(self)),
            SpirvType::Bool => {
    ...
    fn intcast(&mut self, val: Self::Value, dest_ty: Self::Type, is_signed: bool) -> Self::Value {
        ...
        match (self.lookup_type(val.ty), self.lookup_type(dest_ty)) {
            ...
            (SpirvType::Integer(..), SpirvType::Integer(_, dest_signedness)) => {
            ...
            (val_ty, dest_ty_spv) => self.fatal(format!(
                "TODO: intcast not implemented yet: val={val:?} val.ty={val_ty:?} \
                 dest_ty={dest_ty_spv:?} is_signed={is_signed}"
            )),
        }
    }
                ...
                if rt_args_count > 0 {
                        ...
                        rt_arg_new_calls.zip(stores_to_rt_args_array).enumerate()
                    {
                        ...
                        match store_to_rt_args_array_insts[..] {
                            ...
                            _ => {
                                return Err(FormatArgsNotRecognized(format!(
                                    "[fmt::rt::Argument; N] stores sequence \
                                     ({store_to_rt_args_array_insts:?})"
                                )));
                            }
                        }
                    }
                ...
                Err(FormatArgsNotRecognized(step)) => {
                    if let Some(current_span) = self.current_span {
                        let mut warn = self.tcx.sess.psess.dcx.struct_span_warn(
                            current_span,
                            "failed to find and remove `format_args!` construction for this \
                             `panic!`",
                        );
                        ...
                        if self.tcx.sess.opts.unstable_opts.inline_mir != Some(false) {
                            warn.note(
                                "missing `-Zinline-mir=off` flag (should've been set by \
                                 `spirv-builder`)",
                            )
                            .help(
                                "check `.cargo` and environment variables for potential overrides",
                            )
                            .help(
                                "(or, if not using `spirv-builder` at all, add the flag manually)",
                            );
                        } else {
