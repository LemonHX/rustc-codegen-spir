...
use either::Either;
use rspirv::{
    binary::{Assemble, Consumer},
    dr::{Block, Instruction, Loader, Module, ModuleHeader, Operand},
    spirv::{Op, StorageClass, Word},
};
    ...
    // NOTE(eddyb) SPIR-T pipeline is entirely limited to this block.
    {
        ...
        let mut module = {
            ...
            match spirt::Module::lower_from_spv_bytes(cx.clone(), spv_bytes) {
                ...
                Err(e) => {
                    ...
                    return Err({
                        let mut err = sess.psess.dcx.struct_err(format!("{e}"));
        ...
        if any_spirt_bugs {
            ...
            if opts.dump_spirt_passes.is_none() {
                note.help(
                    "re-run with `RUSTGPU_CODEGEN_ARGS=\"--dump-spirt-passes=$PWD\"` for more \
                     details",
                );
            }
    ...
    for inst in &output.ext_inst_imports {
        ...
        if ext_inst_set.starts_with(custom_insts::CUSTOM_EXT_INST_SET_PREFIX) {
            ...
            if ext_inst_set == expected {
                return Err(sess.psess.dcx.err(format!(
                    "`OpExtInstImport {ext_inst_set:?}` should not have been left around after \
                     SPIR-T passes"
                )));
            } else {
                return Err(sess.psess.dcx.err(format!(
