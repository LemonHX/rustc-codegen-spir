...
use rspirv::{
    dr::Operand,
    spirv::{Capability, Decoration, Dim, ExecutionModel, FunctionControl, StorageClass, Word},
};
...
use crate::{
    abi::ConvSpirvType,
    attr::{AggregatedSpirvAttributes, Entry, Spanned, SpecConstant},
    builder::Builder,
    builder_spirv::{SpirvValue, SpirvValueExt},
...
impl<'tcx> CodegenCx<'tcx> {
        ...
        for (arg_abi, hir_param) in fn_abi.args.iter().zip(hir_params) {
            match arg_abi.mode {
                ...
                PassMode::Pair(..) => {
                    ...
                    if !matches!(arg_abi.layout.ty.kind(), ty::Ref(..)) {
                        self.tcx.sess.psess.dcx.span_err(
                            hir_param.ty_span,
                            format!(
                                "entry point parameter type not yet supported (`{}` has \
                                 `ScalarPair` ABI but is not a `&T`)",
                                arg_abi.layout.ty
                            ),
                        );
                    }
                ...
                PassMode::Ignore => self.tcx.sess.psess.dcx.span_fatal(
                    hir_param.ty_span,
                    format!(
                        "entry point parameter type not yet supported (`{}` has size `0`)",
                        arg_abi.layout.ty
                    ),
                ),
                _ => span_bug!(
                ...
                Some(deduced) => {
                    self.tcx
                        ...
                        .span_help(
                            storage_class_attr.span,
                            format!(
                                "remove storage class attribute to use `{deduced:?}` as storage \
                                 class"
                            ),
                        );
                }
    ...
    fn declare_shader_interface_for_param(
        ...
        assert_eq!(
            ...
            "`{}` param mismatch in call ABI (is_pair={is_pair}) + SPIR-V type \
             (is_unsized={is_unsized}) vs layout:\n{value_layout:#?}",
        ...
        if let Some(attachment_index) = attrs.input_attachment_index {
            if is_subpass_input && self.builder.has_capability(Capability::InputAttachment) {
                ...
                self.tcx.sess.psess.dcx.span_err(
                    ...
                    "#[spirv(input_attachment_index)] is only valid on Image types with dim = \
                     SubpassData",
                );
            }
            decoration_supersedes_location = true;
            ...
            self.tcx.sess.psess.dcx.span_err(
                hir_param.ty_span,
                "Image types with dim = SubpassData require #[spirv(input_attachment_index)] \
                 decoration",
            );
        }
    ...
    fn check_for_bad_types(
        ...
        fn recurse(cx: &CodegenCx<'_>, ty: Word, has_bool: &mut bool, must_be_flat: &mut bool) {
            match cx.lookup_type(ty) {
                ...
                SpirvType::Integer(..) | SpirvType::Float(64) => *must_be_flat = true,
