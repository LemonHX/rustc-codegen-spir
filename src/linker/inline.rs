...
use rspirv::{
    dr::{Block, Function, Instruction, Module, ModuleHeader, Operand},
...
use super::{
    apply_rewrite_rules, get_name, get_names, ipo::CallGraph, simple_passes::outgoing_edges,
};
    ...
    if !inlined_to_legalize_dont_inlines.is_empty() {
        ...
        for f in inlined_to_legalize_dont_inlines {
            sess.psess.dcx.warn(format!(
                "`#[inline(never)]` function `{}` needs to be inlined because it has illegal \
        ...
        for next in calls(&module.functions[current], func_to_index) {
            if discovered[next] {
                ...
                *has_recursion = Some(sess.psess.dcx.err(format!(
                    "module has recursion, which is not allowed: `{current_name}` calls \
                     `{next_name}`"
                )));
                break;
            }
