...
use crate::{
    abi::ConvSpirvType,
    builder_spirv::{BuilderCursor, SpirvValue, SpirvValueExt},
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    ...
    pub(crate) fn struct_gep(
        &mut self,
        ty: <crate::Builder<'a, 'tcx> as BackendTypes>::Type,
        ptr: SpirvValue,
        idx: u64,
    ) -> <crate::Builder<'a, 'tcx> as BackendTypes>::Value {
    ...
    #[track_caller]
    pub fn struct_err(&self, msg: impl Into<DiagMessage>) -> Diag<'_, ErrorGuaranteed> {
        if let Some(current_span) = self.current_span {
